...
function htmlDoc({ pr, files, commits }, existingHtml = "") {
  ...
  let evidenceSection = "";
  if (existingHtml && existingHtml.includes("<h2>Evidence</h2>")) {
    const match = existingHtml.match(/<section class="card">\s*<h2>Evidence<\/h2>[\s\S]*?<\/section>/);
    if (match) {
      evidenceSection = "\n      " + match[0];
    }
  }
...
function mdDoc({ pr, files, commits }, existingMd = "") {
  const title = pr.title || "Untitled PR";
  const description = extractDescription(pr.body);
  const mergedAt = pr.merged_at || (pr.merge_commit_sha ? pr.merged_at : null);
  const state = mergedAt ? "merged" : pr.state;
...
`;
  ...
  let evidenceSection = "";
  if (existingMd && existingMd.includes("## Evidence")) {
    const idx = existingMd.indexOf("## Evidence");
    evidenceSection = "\n" + existingMd.slice(idx).trim() + "\n";
  }

  return baseDoc + evidenceSection;
}
...
async function main() {
  ...
  if (SINGLE_PR !== null) {
    ...
    try {
      ...
      if (!DRY_RUN) {
        const existingMd = fileExists(mdPath) ? readFileSync(mdPath, "utf8") : "";
        const existingHtml = fileExists(htmlPath) ? readFileSync(htmlPath, "utf8") : "";
        writeFileSync(mdPath, mdDoc(data, existingMd), "utf8");
        writeFileSync(htmlPath, htmlDoc(data, existingHtml), "utf8");
  ...
  for (const pr of aoPRs) {
    ...
    try {
      ...
      const existingMd = mdExists ? readFileSync(mdPath, "utf8") : "";
      const existingHtml = htmlExists ? readFileSync(htmlPath, "utf8") : "";
      writeFileSync(mdPath, mdDoc(data, existingMd), "utf8");
      writeFileSync(htmlPath, htmlDoc(data, existingHtml), "utf8");
      generated++;
    } catch (err) {
