...
| 3 | **Approved review** | CodeRabbit APPROVED, or a documented current-head substitute review under the slow-bot/no-auto-approve policy; any CHANGES_REQUESTED still blocks |
...
# 4. Cross-reference review state (do NOT trust gh pr checks)
...
| 3 | "CodeRabbit: pass" (MISLEADING) | Latest non-COMMENTED CodeRabbit review = APPROVED, or a documented current-head substitute review under policy |
...
**Rule**: A PR is 6-green ONLY when all 6 gates show PASS at the current head. Never report 6-green status based on `gh pr checks` output alone.
...
**Squash before final merge**: When all 6-green conditions are met, squash all commits into ONE before pushing:
...
Then merge from the authorized human-facing session (for example, `gh pr merge N --squash --admin`).
