...
- **Bar-raising (UI / interactive):** MUST provide **video evidence (.mp4/.gif/.cast)** of key flows + **before/after** states. Screenshots alone are INSUFFICIENT. Direct all agents to follow repository standards in `skills/ui-video-evidence/SKILL.md` and `skills/tmux-video-evidence/SKILL.md`. Map claims to gist steps / logs / media (`docs/evidence/reviewer-checklist.md`).
