...
update_metadata_key() {
  ...
  # Escape special sed characters in replacement: &, \, and | (the latter
  # because the sed command uses | as its own delimiter; unescaped | in the
  # replacement would be parsed as an additional delimiter, breaking the cmd)
