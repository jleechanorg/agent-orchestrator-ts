...
- The Skeptic Gate workflow scopes concurrency per PR and only accepts fresh request-bound, SHA-bound comments that contain the `<!-- skeptic-agent-verdict -->` marker, preventing ordinary PR notes from satisfying the gate.
- A PASS verdict must include all eight hidden gate PASS markers; FAIL and SKIPPED remain explicit fail-closed verdicts.
