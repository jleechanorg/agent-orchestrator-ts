...
The call site for the local cron in `packages/core/src/lifecycle-manager.ts` at line 2799 must invoke `runLocalSkepticCron` unconditionally for any configured project without guarding on project backfill parameters (e.g. `backfillAllPRs`).

### Invariant B: Open batch boundary without recency/draft filter
The top-level batch collection `eligiblePRs` at line 314 of `packages/core/src/skeptic-cron-local.ts` must filter out only draft pull requests (`pr.isDraft`). No age-based, recency-based, or activity-based filtering is allowed at this public boundary.
...
* **Test Reference**: `packages/core/src/__tests__/skeptic-cron-local.test.ts` line 1173 (`evaluates PRs modified more than 24 hours ago if they have a trigger comment`).
...
* **Test Reference**: `packages/core/src/__tests__/skeptic-cron-local.test.ts` line 1200 (`evaluates PRs modified within the last 24 hours`).
...
* **Test Reference**: `packages/core/src/__tests__/skeptic-cron-local.test.ts` line 1450 (`falls back to 24h PR age check if listPRComments is missing`).
...
- [ ] **Invariant A Met**: Call site in `lifecycle-manager.ts` at line 2799 does not check `backfillAllPRs`.
- [ ] **Invariant B Met**: `eligiblePRs` filter in `skeptic-cron-local.ts` at line 314 filters only drafts (`!pr.isDraft`).
...
1. **PR #497 Call Site Decoupling**: [PR #497](https://github.com/jleechanorg/agent-orchestrator/pull/497), Merge Commit: `e1f11d0033e0a7a7c57ff981a43de26389fb1af8`, File: `packages/core/src/lifecycle-manager.ts:2799`
2. **PR #654 Recency Filter Re-coupling**: [PR #654](https://github.com/jleechanorg/agent-orchestrator/pull/654), Merge Commit: `8dfd5c207f2963e2ff9964f1d6d8ae5855538a86`, File: `packages/core/src/skeptic-cron-local.ts` (diff lines 227-242)
3. **PR #661 Decoupling Restoration**: [PR #661](https://github.com/jleechanorg/agent-orchestrator/pull/661), Head Commit: `1a9767f55c07d3c848cbdcdff421b50faff7c68b`, File: `packages/core/src/skeptic-cron-local.ts:237-279`, `packages/core/src/skeptic-cron-local.ts:314`
4. **Decoupling Regression Tests**: Head Commit: `1a9767f55c07d3c848cbdcdff421b50faff7c68b`, File: `packages/core/src/__tests__/skeptic-cron-local.test.ts:1173-1221`, `packages/core/src/__tests__/skeptic-cron-local.test.ts:1419-1498`
