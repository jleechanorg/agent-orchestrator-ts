...
| 11 | `packages/mobile/app.json` | 18–19, 33 | (c) composio-collision (branding) | **MEDIUM** | iOS `bundleIdentifier` and Android `package` use `com.composio.*` while the app is named “Agent Orchestrator” under the fork. May be intentional legacy; if the fork ships independently, identifiers collide with Composio namespace expectations. **Recommendation:** confirm policy; consider `org.jleechanorg.ao` (or similar) for store builds if legally/product-appropriate. |
