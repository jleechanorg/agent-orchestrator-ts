...
async function handleUrlStart(
  ...
  if (existsSync(configPath)) {
    ...
    return {
      config: writeYamlConfig(stagingConfigPath, readFileSync(configPath, "utf-8")),
      parsed,
      autoGenerated: false,
    };
  }
  ...
  if (existsSync(configPathAlt)) {
    console.log(chalk.yellow(`  Migrating repo-local config into staging: ${stagingConfigPath}`));
    return {
      config: writeYamlConfig(stagingConfigPath, readFileSync(configPathAlt, "utf-8")),
      parsed,
      autoGenerated: false,
    };
  }
      ...
      async (
        ...
        try {
          ...
          if (projectArg && isRepoUrl(projectArg)) {
            ...
            let configPath: string | undefined = process.env["AO_CONFIG_PATH"]
            ...
            try {
              if (process.env["AO_CONFIG_PATH"]) {
                ...
                const managedConfigPath = findManagedConfigFile();
                if (!managedConfigPath) {
                  throw new ConfigNotFoundError();
                }
                loadedConfig = loadConfig(managedConfigPath);
              }
            } catch (err) {
