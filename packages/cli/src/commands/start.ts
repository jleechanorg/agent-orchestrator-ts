 ...
 *
 * When `allowMainRepo` is true (set by the --allow-main-repo CLI flag,
 * bd-cj5s), the guard is a no-op. The flag is an explicit per-invocation
 * opt-in that replaces the legacy AO_MAIN_REPO env-var workaround.
 */
function guardMainRepo(resolvedPath: string, mainRepoPath: string, allowMainRepo = false): void {
  if (allowMainRepo) {
    return;
  }
  if (
    ...
    throw new Error(
      `Refusing to operate on the main repo (${resolvedPath}). ` +
        ...
        `or pass --allow-main-repo to bypass this guard.`,
    );
  }
}
      ...
      async (
        ...
        try {
          ...
          if (projectArg && isRepoUrl(projectArg)) {
            ...
            const result = await handleUrlStart(projectArg, opts?.allowMainRepo);
