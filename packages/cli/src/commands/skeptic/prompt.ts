...
export function buildSkepticPrompt(
  ...
  return [
    ...
    "    - FAIL if Terminal media is provided but the URL does not point to a .mp4, .gif, or .cast file (must be video evidence).",
    "    - FAIL if UI media is provided but the URL does not point to a .mp4, .gif, or .cast file (screenshots alone are insufficient for UI claims).",
    "    - FAIL if media is provided but lacks a caption or description tying it to the commit SHA.",
    "    - PASS if evidence shows real command output, real test results, or real authentic video evidence (.mp4/.gif/.cast) with non-placeholder URLs",
