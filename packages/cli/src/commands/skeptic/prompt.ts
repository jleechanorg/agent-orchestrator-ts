...
export function buildSkepticPrompt(
  ...
  return [
    ...
    "    - FAIL if Terminal media is provided but the URL is not HTTPS or does not point to a .mp4, .gif, or .cast file (must be video evidence per repo skill: skills/tmux-video-evidence/SKILL.md).",
    "    - FAIL if UI media is provided but the URL is not HTTPS or does not point to a .mp4 or .gif file (screenshots alone are insufficient per repo skill: skills/ui-video-evidence/SKILL.md).",
    "    - FAIL if media is provided but lacks a caption or description tying it to the commit SHA.",
    "    - PASS if evidence shows real command output, real test results, or real authentic video evidence with HTTPS non-placeholder URLs and required caption/SHA linkage as specified in skills/evidence-standards/SKILL.md.",
