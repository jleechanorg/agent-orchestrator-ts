...
export function buildSkepticPrompt(
  ...
  return [
    ...
    "    If the PR description contains a '## Goals' or '## Goal' section with bullet points:",
    "    12a. Extract each bullet/numbered item from the Goals section",
    "    12b. For each goal bullet, verify the diff contains code changes that implement it",
    "    12c. FAIL if any goal bullet has NO corresponding implementation in the diff",
    "    12d. A goal that only has test changes is NOT implemented (tests prove behavior, not create it)",
    ...
    "Include a '## Goals Verification' section in your FAIL report when Rule 12 gaps are found.",
    "",
