 ...
 *   3. fallback — direct connection to `:DIRECT_TERMINAL_PORT/ws`
 *
...
export function createSinglePortServer(config: SinglePortConfig): SinglePortServer {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    const target =
      pathname === MUX_PATH
        ? { host: "127.0.0.1", port: directTerminalPort, path: "/ws" }
...
function main(): void {
  ...
  void proxy.listen().then(() => {
    console.log(
      `[single-port] listening on ${config.port}; HTTP → 127.0.0.1:${config.nextInternalPort}; ${MUX_PATH} → 127.0.0.1:${config.directTerminalPort}/ws`,
    );
  });
