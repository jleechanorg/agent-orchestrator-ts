...
function extractCost(lines: JsonlLine[]): CostEstimate | undefined {
  ...
  // Rough estimate when no direct cost data — uses Gemini 2.0 Flash pricing
  // as a baseline ($0.10/M input, $0.40/M output). Will be inaccurate for
  // other Gemini models but provides a useful order-of-magnitude signal.
  if (totalCost === 0 && (inputTokens > 0 || outputTokens > 0)) {
    totalCost = (inputTokens / 1_000_000) * 0.1 + (outputTokens / 1_000_000) * 0.4;
  }
