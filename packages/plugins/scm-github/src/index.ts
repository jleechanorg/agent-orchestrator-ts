...
function createGitHubSCM(config?: Record<string, unknown>): SCM {
  ...
  return {
    ...
    async checkoutPR(pr: PRInfo, workspacePath: string): Promise<boolean> {
      ...
      if (dirty) {
        // AO-managed runtime files (written by agent-base plugin at session startup)
        // that are expected to differ from committed versions.
        const AO_MANAGED = new Set([
          ...
          "AGENTS.md",           // auto-generated by ao spawn from template
        ]);
        ...
        const remainingDirty = await git(["status", "--porcelain"], workspacePath);
        if (remainingDirty) {
          throw new Error(
