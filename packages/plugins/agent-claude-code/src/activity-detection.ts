 ...
 *     (waiting_input/blocked) from hook/terminal sources
 ...
 * as JSONL types. `waiting_input` flows through hook/terminal →
 * AO activity JSONL path. `blocked` is detected from native JSONL via
 * `{type:"system", level:"error"}` (Claude's api_error shape) or from
...
export async function getClaudeActivityState(
  ...
  // the wrong session's JSONL whenever its sibling has just written.
  const rawUuid = session.metadata?.["claudeSessionUuid"] ?? ((session.agentInfo as { metadata?: Record<string, unknown> } | undefined)?.metadata)?.["claudeSessionUuid"];
  const preferredUuid =
  ...
  if (sessionFile) {
    ...
    if (entry) {
      ...
      if (session.createdAt && entry.modifiedAt < session.createdAt) {
        ...
        // activity-JSONL pipeline for a terminal/hook-derived answer; if that's
  ...
  // Fallback: check AO activity JSONL (hook/terminal-derived) for
