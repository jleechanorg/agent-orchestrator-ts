...
if (mergePattern.test(cleanCommand)) {
  const allowMerge = process.env.AO_ALLOW_GH_PR_MERGE === "1";
  if (hookEvent !== "PostToolUse" && !allowMerge) {
    process.stdout.write(JSON.stringify({
      hookSpecificOutput: {
        hookEventName: "PreToolUse",
        permissionDecision: "deny",
        permissionDecisionReason: "Blocked by AO policy: agents must not run gh pr merge. Leave merge to orchestrator/human."
      }
    }) + "\\n");
    process.exit(0);
  }
...
// All metadata writers run in PostToolUse only.
// Allow PreToolUse (hookEvent empty or "PreToolUse") to fall through to guards above.
...
const allowedBases = [join(home, ".ao"), join(home, ".agent-orchestrator"), join(home, ".ao-sessions"), os.tmpdir()];
...
if (mergePattern.test(cleanCommand)) {
  const allowMerge = process.env.AO_ALLOW_GH_PR_MERGE === "1";
  if (allowMerge && hookEvent === "PostToolUse") {
...
function classifyTerminalOutput(_terminalOutput: string): ActivityState {
  // #1941: Claude activity is derived from platform-event hooks.
  // classifyTerminalOutput is kept as a stable "idle" stub fallback.
  return "idle";
}
...
/**
 * Shared helper to setup hooks in a workspace.
 * Writes metadata-updater + activity-updater scripts and updates settings.json.
...
async function setupHookInWorkspace(workspacePath: string): Promise<void> {
  ...
  let metadataScriptPath: string;
  let activityScriptPath: string;
  let metadataCommand: string;
  let activityCommand: string;

  if (isWindows()) {
    metadataScriptPath = join(claudeDir, "metadata-updater.cjs");
    activityScriptPath = join(claudeDir, "activity-updater.cjs");
    metadataCommand = "node .claude/metadata-updater.cjs";
    activityCommand = "node .claude/activity-updater.cjs";
  } else {
    metadataScriptPath = join(claudeDir, "metadata-updater.sh");
    activityScriptPath = join(claudeDir, "activity-updater.sh");
  ...
  let metadataScriptIsSymlink = false;
  try {
    const scriptSt = await lstat(metadataScriptPath);
    metadataScriptIsSymlink = scriptSt.isSymbolicLink();
    if (metadataScriptIsSymlink) {
      console.warn(`[agent-claude-code] ${metadataScriptPath} is a symlink — skipping hook script write to avoid clobbering target`);
    }
  } catch {
    // File does not exist — safe to write
  }

  let activityScriptIsSymlink = false;
  try {
    const scriptSt = await lstat(activityScriptPath);
    activityScriptIsSymlink = scriptSt.isSymbolicLink();
    if (activityScriptIsSymlink) {
      console.warn(`[agent-claude-code] ${activityScriptPath} is a symlink — skipping hook script write to avoid clobbering target`);
    }
  } catch {
    // File does not exist — safe to write
  }

  // Write the updater scripts only if content changed (idempotent) and not a symlink
  if (isWindows()) {
    if (!metadataScriptIsSymlink && (!existsSync(metadataScriptPath) || (await readFile(metadataScriptPath, "utf-8")) !== METADATA_UPDATER_SCRIPT_NODE)) {
      await writeFile(metadataScriptPath, METADATA_UPDATER_SCRIPT_NODE, "utf-8");
    }
    if (!activityScriptIsSymlink && (!existsSync(activityScriptPath) || (await readFile(activityScriptPath, "utf-8")) !== ACTIVITY_UPDATER_SCRIPT_NODE)) {
      await writeFile(activityScriptPath, ACTIVITY_UPDATER_SCRIPT_NODE, "utf-8");
    }
  } else {
    if (!metadataScriptIsSymlink && (!existsSync(metadataScriptPath) || (await readFile(metadataScriptPath, "utf-8")) !== METADATA_UPDATER_SCRIPT)) {
      await writeFile(metadataScriptPath, METADATA_UPDATER_SCRIPT, "utf-8");
    }
    if (!activityScriptIsSymlink && (!existsSync(activityScriptPath) || (await readFile(activityScriptPath, "utf-8")) !== ACTIVITY_UPDATER_SCRIPT)) {
      await writeFile(activityScriptPath, ACTIVITY_UPDATER_SCRIPT, "utf-8");
    }
  }

  // Always ensure execute bit is set, but skip if the hook script is itself a symlink
  if (!isWindows()) {
    const mSt = await lstat(metadataScriptPath).catch(() => null);
    if (mSt && !mSt.isSymbolicLink()) {
      await chmod(metadataScriptPath, 0o755);
    }
    const aSt = await lstat(activityScriptPath).catch(() => null);
    if (aSt && !aSt.isSymbolicLink()) {
      await chmod(activityScriptPath, 0o755);
    }
  }
