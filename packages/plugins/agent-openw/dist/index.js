    ...
    return {
        ...
        getEnvironment(config) {
            ...
            if (waferKey) {
                ...
                env["OPENAI_API_KEY"] = "";
                console.error("[ao-plugin-agent-openw] WAFER_API_KEY not found. Set WAFER_API_KEY in your environment or in a file listed under envSource in agent-orchestrator.yaml.");
            }
        ...
        detectActivity(terminalOutput) {
            ...
            if (CONFIRMATION_WIDGET_RE.test(lastLine))
        ...
        async getActivityState(session, readyThresholdMs) {
            ...
            if (!running)
                return { state: "exited", timestamp: exitedAt };
            try {
                const { stdout } = await execFileAsync("opencode", ["session", "list", "--format", "json"], { timeout: 30_000 });
