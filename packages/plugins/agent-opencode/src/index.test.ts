...
describe("getLaunchCommand", () => {
  ...
  it("uses --append-system-prompt with systemPrompt", () => {
    const cmd = agent.getLaunchCommand(
  ...
  it("escapes single quotes in systemPrompt", () => {
    ...
    expect(cmd).toContain("--append-system-prompt 'it'\\''s important'");
  });
  ...
  it("generates command with systemPromptFile via shell substitution", () => {
    ...
    expect(cmd).toContain("--append-system-prompt \"$(cat '/tmp/prompt.md')\"");
  });
  ...
  it("systemPromptFile takes precedence over systemPrompt", () => {
    ...
    expect(cmd).toContain("--append-system-prompt \"$(cat '/tmp/file-prompt.md')\"");
  ...
  it("combines systemPromptFile with subagent and prompt", () => {
    ...
    expect(cmd).toContain("--append-system-prompt \"$(cat '/tmp/orchestrator.md')\"");
    expect(cmd).toContain("--agent 'sisyphus'");
    expect(cmd).toContain("--prompt 'fix the bug'");
  });
