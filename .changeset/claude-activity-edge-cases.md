...
Harden Claude Code activity detection against five real-world edge cases identified during upstream PR #1927's analysis:
...
193/193 plugin tests pass. Backwards-compatible API addition: new helper `resolveWorkspaceForClaude` is re-exported from `index.ts` for downstream consumers.
